...
Use this repository's `image-reducer` CLI as a localhost reverse proxy. It filters only repeated `data:image/*;base64,...` history and never stores image bytes or credentials.

## Start

1. Identify the existing Responses-compatible upstream base URL and its API-key environment variable. Do not print the key.
2. Run `node ./bin/image-reducer.mjs start --listen 127.0.0.1:8787 --upstream <upstream-base-url>` from the repository root.
